...
public class Telemetry {
    ...
    // WPILib Field2d object for AdvantageScope visualization
    ...
    public Telemetry(double maxSpeed) {
        MaxSpeed = maxSpeed;
        if (Constants.ENABLE_SIGNAL_LOGGER) SignalLogger.start();

        // Publish Field2d to SmartDashboard (AdvantageScope will see this)
        SmartDashboard.putData("Field", field);

        /* Set up the module state Mechanism2d telemetry */
        for (int i = 0; i < 4; ++i) {
            TelemetryManager.putData("Module " + i, m_moduleMechanisms[i]);
        }
    ...
    private static final int SIGNAL_LOGGER_PERIOD = 5; // Log to hoot file every 5 loops (100ms)
    ...
    public void telemeterize(SwerveDriveState state) {
        ...
        /* Telemeterize the swerve drive state to NetworkTables (always - needed for driving) */
        ...
        // Update Field2d (SmartDashboard + AdvantageScope)
        field.setRobotPose(state.Pose);

        /* Write to SignalLogger/hoot file at a reduced rate */
        if (Constants.ENABLE_SIGNAL_LOGGER && m_telemetryCounter % SIGNAL_LOGGER_PERIOD == 0) {
    ...
    public void override_field_position(Pose2d pose) {
